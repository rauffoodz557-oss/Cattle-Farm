...
function ensureMonth(){
  ...
  data.customers.forEach(c => {
    if(!data.months[currentMonth][c.id]) data.months[currentMonth][c.id] = { milk: {}, rate: c.rate || 200 };
  });
  save();
}
...
function renderMilk(){
 ...
 data.customers.forEach((c, i) => {
   ...
   const rate = rec.rate ?? c.rate ?? 200, amount = total * Number(rate || 0); 
   grand += total;
   body += `<td class="total-cell" id="total-${c.id}">${fmt(total)}</td>
            ...
            <td class="action-cell"><button class="print-btn" data-print-id="${c.id}">Print</button> <button class="btn red del-btn" data-del-id="${c.id}">Delete</button></td></tr>`;
 });
...
function getCustomerMonthTotal(c){
  ...
  return { total, rate, amount: total * rate };
}

function renderRecoverySheet(){
  const list = $("recoveryList"), foot = $("recoveryFoot");
  if(!list) return;
  let rows = "", grandMilk = 0, grandAmount = 0;
  data.customers.forEach((c, i) => {
    const { total, rate, amount } = getCustomerMonthTotal(c);
    grandMilk += total; grandAmount += amount;
    rows += `<tr><td>${i + 1}</td><td>${esc(c.name)}</td><td>${fmt(total)}</td><td>${fmt(rate)}</td><td>${fmt(amount)}</td><td></td><td><button class="btn red del-btn" data-del-id="${c.id}">Delete</button></td></tr>`;
  });
  list.innerHTML = rows;
  if(foot){
    foot.innerHTML = `<tr><td></td><td style="direction:rtl">کل / Grand Total</td><td>${fmt(grandMilk)}</td><td></td><td>${fmt(grandAmount)}</td><td></td><td></td></tr>`;
  }
}
...
function printRecoverySheet(){
  ...
  data.customers.forEach((c, i) => {
    const { total, rate, amount } = getCustomerMonthTotal(c);
    grandMilk += total; grandAmount += amount;
    rows += `<tr><td>${i + 1}</td><td>${esc(c.name)}</td><td>${fmt(total)}</td><td>${fmt(rate)}</td><td>${fmt(amount)}</td><td></td></tr>`;
  });
  ...
  <table class="rs-table">
    <thead><tr><th>Sr.No</th><th>Customer Name</th><th>Milk KG</th><th>Rate</th><th>Total Amount</th><th>Recovery</th></tr></thead>
    <tbody>${rows}</tbody>
    <tfoot><tr><td colspan="2">Grand Total</td><td>${fmt(grandMilk)}</td><td></td><td>${fmt(grandAmount)}</td><td></td></tr></tfoot>
  </table>
...
const PAGE_SIZE_CSS = { A6: "A6 portrait", A5: "A5 portrait", A4: "A4 portrait", Letter: "8.5in 11in" };
...
function openPrintPreview(html, options){
  ...
  content.innerHTML = `<div id="previewScaleInner">${html}</div>`;
  if(afterInsertCallback) afterInsertCallback(content);
  modal.classList.remove("hidden");

  const pageSizeSelect = $("previewPageSize"), zoomLevel = $("zoomLevel");
  ...
  function applyDynamicPage(){
    ...
    styleTag.textContent = `@page dynamicPrintPage{size:${PAGE_SIZE_CSS[pageSize] || PAGE_SIZE_CSS.A4};margin:6mm} #printArea{page:dynamicPrintPage}`;
    content.style.maxWidth = PAGE_PREVIEW_WIDTH[pageSize] || "500px";
  }

  function applyScale(){
    const inner = document.getElementById("previewScaleInner");
    if(inner){ inner.style.transformOrigin = "top center"; inner.style.transform = `scale(${scale})`; }
  ...
  $("confirmPrintBtn").onclick = () => {
    if($("printArea")){
      $("printArea").innerHTML = `<div style="transform:scale(${scale});transform-origin:top center;">${html}</div>`;
      if(afterInsertCallback) afterInsertCallback($("printArea"));
    }
...
function printCustomer(id){
 ...
 openPrintPreview(html, {
   ...
   afterInsertCallback: container => {
     createQR(container.querySelector('[data-qr-type="bill"]'), billUrl);
